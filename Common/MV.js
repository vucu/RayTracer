...
// MV.js - The textbook author's example Matrix / Vector math library for Javascript projects.  Whereas for C++ projects, 
//         we will instead use Eigen, a popular matrix library.
//----------------------------------------------------------------------------
//
//  Helper functions
//
...
function vec4( x = 0.0, y = 0.0, z = 0.0, w = 1.0 )
{
    return [ x, y, z, w ];
    // var result = _argumentsToArray( arguments );
...
function add( u, v )
{
    ...
    if ( u.matrix && v.matrix ) {
        ...
        if ( u.length != v.length ) {
            throw "add(): vectors are not the same dimension. The vectors are " + u + " and " + v;
        }
...
{ 
    ...
    if ( !isFinite(len) ) {
        console.log("log start 2. Error!");
        console.log(u);
        console.log(dot(u,u));
        throw "normalize: vector " + u + " has zero length. Length is " + len;
    }
...
function clamp01(u) {
    var v = [];
    for (var i=0; i<u.length; i++) {
        v[i] = Math.min(Math.max(u[i], 0.0), 1.0);
        if (v[i]<0.0) throw "Clamp min unsuccessfully"
        if (v[i]>1.0) throw "Clamp max unsuccessfully"
    }
